...
export default function Home() {
  return (
    <main className="min-h-screen relative border-x border-border-primary max-w-[1600px] mx-auto bg-surface-primary">
      <LayeredHero />
      ...
      {/* ═══════════════════════════════════════════════════════════════
          ACTIVITIES
          ═══════════════════════════════════════════════════════════════ */}
      <section id="community" className="px-6 md:px-12 lg:px-24 pt-24 pb-32 border-b border-border-primary relative bg-surface-secondary text-text-on-dark">
        <div className="relative z-10 mb-20 flex flex-col md:flex-row md:items-end justify-between gap-8 border-l border-[rgba(0,0,0,0.1)] pl-6 md:pl-12">
          <div>
            <span className="font-mono text-xs text-text-on-dark/70 tracking-[0.2em] uppercase mb-4 block">
              01 // The Proving Grounds
            </span>
            <h2 className="text-4xl md:text-5xl font-mono text-text-on-dark uppercase tracking-tight">Activities</h2>
          </div>
          <p className="font-mono text-sm text-text-on-dark/70 max-w-xs md:text-right uppercase tracking-wider">
            Where theory hits the board. Here&apos;s what keeps the forum running week to week.
          </p>
        </div>
        
        <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-border-primary border border-border-primary shadow-xl">
          {[
            { 
              title: "Tournaments", 
      ...
      {/* ═══════════════════════════════════════════════════════════════
          ABOUT
          ═══════════════════════════════════════════════════════════════ */}
      <section className="px-6 md:px-12 lg:px-24 py-24 md:py-32 relative border-b border-border-primary bg-surface-primary overflow-hidden">
        {/* Subtle 8x8 micro-grid decorative motif in corner */}
        <div aria-hidden="true" className="absolute top-12 right-12 opacity-10 pointer-events-none grid grid-cols-8 grid-rows-8 w-64 h-64 border border-border-primary">
          {Array.from({length: 64}).map((_, i) => (
            <div key={i} className={`border border-border-primary/30 ${i % 2 === 0 ? 'bg-surface-inverse' : 'bg-transparent'} ${(i === 27 || i === 36) ? '!bg-surface-inverse !opacity-100' : ''}`} />
          ))}
        </div>
        ...
        <div className="max-w-5xl mx-auto relative z-10 grid md:grid-cols-12 gap-12 md:gap-8">
          ...
          <div className="md:col-span-5 flex flex-col justify-between">
            <div>
              ...
              <h2 className="text-4xl md:text-5xl font-mono uppercase tracking-tight text-text-primary mb-8">Founded on the 64 Squares</h2>
            </div>
            
            <div className="hidden md:flex border-l border-border-primary pl-6 py-2 flex-col gap-2">
              <span className="font-mono text-[10px] text-text-secondary tracking-widest">[ COORD: E4-C5 ]</span>
              <span className="font-mono text-[10px] text-text-secondary tracking-widest">EST. [DATA]</span>
            </div>
          </div>
          ...
          <div className="md:col-span-7 space-y-12">
            <div className="chess-cell-dark p-8 md:p-12 relative group">
              ...
              <blockquote className="border-l border-border-inverse pl-6 md:pl-10 cell-border">
            ...
            <div className="space-y-6 text-text-secondary font-mono text-sm leading-relaxed uppercase tracking-wider pl-4 border-l border-border-primary/50">
              <p>
      ...
      {/* ═══════════════════════════════════════════════════════════════
          CONTACT
          ═══════════════════════════════════════════════════════════════ */}
      <section className="px-6 md:px-12 lg:px-24 pt-24 pb-32 bg-surface-secondary text-text-on-dark relative text-center">
        <div className="max-w-4xl mx-auto border border-border-primary p-8 md:p-16 bg-board-light text-text-on-light shadow-2xl relative">
          <div className="absolute top-4 left-4 font-mono text-[10px] text-text-on-light/50 tracking-widest">[03]</div>
          <div className="absolute top-4 right-4 font-mono text-[10px] text-text-on-light/50 tracking-widest">[ENDGAME]</div>
          
          <h2 className="text-4xl md:text-5xl lg:text-7xl font-mono uppercase tracking-tight text-text-on-light mb-6 mt-8">Take Your Seat</h2>
          <p className="font-mono text-sm md:text-base text-text-on-light/70 mb-16 uppercase tracking-wider">
            Join the conversation. We&apos;re always looking for new challengers.
          </p>
          
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-px bg-border-primary border border-[rgba(0,0,0,0.1)] mb-16 text-left shadow-sm">
          ...
          <button className="px-12 py-4 font-mono text-xs tracking-widest uppercase shadow-[4px_4px_0_0_rgba(0,0,0,0.2)] bg-board-active text-text-on-active transition-colors hover:bg-board-light hover:text-text-on-light border border-[rgba(0,0,0,0.1)]">
