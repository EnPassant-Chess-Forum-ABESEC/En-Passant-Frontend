...
export default function RecruitmentPage() {
  ...
  return (
    <main className="min-h-screen relative border-x border-ep-border max-w-[1600px] mx-auto bg-ep-bg pb-24">
      <Logo />
      
      {/* 00 - RECRUITMENT HERO */}
      <section className="px-6 md:px-12 lg:px-24 pt-32 pb-24 border-b border-ep-border relative bg-ep-bg">
        <div className="max-w-5xl mx-auto relative z-10 flex flex-col items-start">
          <span className={`font-mono text-xs text-ep-gray tracking-[0.2em] uppercase mb-6 block border border-ep-border px-4 py-2 motion-reduce:opacity-100 ${heroBaseAnim} motion-safe:[animation-delay:0ms]`}>
            [RECRUITMENT // {recruitmentConfig.year}]
          </span>
          <h1 className={`text-5xl md:text-7xl lg:text-8xl font-mono text-ep-black uppercase tracking-tight leading-none mb-8 motion-reduce:opacity-100 ${heroBaseAnim} motion-safe:[animation-delay:150ms]`}>
            THE NEXT MOVE<br />IS YOURS.
          </h1>
          <p className={`font-mono text-sm md:text-base text-ep-gray max-w-2xl leading-relaxed uppercase tracking-wider mb-12 motion-reduce:opacity-100 ${heroBaseAnim} motion-safe:[animation-delay:250ms]`}>
            EnPassant Chess Forum is looking for the next group of students who will help organize, moderate, and grow chess at ABESEC.
          </p>
          
          <div className={`flex flex-col md:flex-row gap-8 md:gap-16 border-l border-ep-border pl-6 md:pl-10 mb-16 motion-reduce:opacity-100 ${heroBaseAnim} motion-safe:[animation-delay:350ms]`}>
            <div className={`motion-reduce:opacity-100 ${hasJustTransitioned ? 'opacity-[0.15] motion-safe:animate-fade-in' : 'opacity-0 motion-safe:animate-fade-in'} motion-safe:[animation-delay:400ms]`}>
              <span className="font-mono text-[10px] text-ep-primary tracking-widest block mb-1">STATUS</span>
              <span className="font-mono text-sm text-ep-black tracking-widest uppercase font-bold">APPLICATIONS OPEN</span>
            </div>
            <div className={`motion-reduce:opacity-100 ${hasJustTransitioned ? 'opacity-[0.15] motion-safe:animate-fade-in' : 'opacity-0 motion-safe:animate-fade-in'} motion-safe:[animation-delay:500ms]`}>
              <span className="font-mono text-[10px] text-ep-primary tracking-widest block mb-1">FOR</span>
              <span className="font-mono text-sm text-ep-black tracking-widest uppercase font-bold">ABESEC STUDENTS</span>
            </div>
            <div className={`motion-reduce:opacity-100 ${hasJustTransitioned ? 'opacity-[0.15] motion-safe:animate-fade-in' : 'opacity-0 motion-safe:animate-fade-in'} motion-safe:[animation-delay:600ms]`}>
              <span className="font-mono text-[10px] text-ep-primary tracking-widest block mb-1">COMMUNITY</span>
              <span className="font-mono text-sm text-ep-black tracking-widest uppercase font-bold">400+ MEMBERS</span>
            </div>
          </div>
          
          <div className={`bg-ep-bg-deep border border-ep-border p-8 md:p-12 w-full max-w-2xl relative overflow-hidden group motion-reduce:opacity-100 ${heroBaseAnim} motion-safe:[animation-delay:700ms]`}>
            <div className="relative z-10 flex flex-col items-start gap-4">
              <span className="font-mono text-[10px] tracking-widest uppercase text-ep-black">
                {recruitmentConfig.isOpen ? 'RECRUITMENT IS OPEN' : 'RECRUITMENT IS CLOSED'}
              </span>
              <div className="bg-ep-bg-raised border border-ep-border px-8 py-4 w-full cursor-not-allowed">
                <span className="font-mono text-sm tracking-widest uppercase text-ep-gray block text-center opacity-80">
      ...
      {/* 01 - COMMUNITY != FORUM */}
      <RevealSection className="px-6 md:px-12 lg:px-24 py-24 border-b border-ep-border bg-ep-bg-deep">
        <div className="max-w-5xl mx-auto">
          <h2 className="text-3xl md:text-4xl font-mono uppercase tracking-tight text-ep-black mb-12 text-center md:text-left">
            EVERY FORUM MEMBER IS PART OF THE COMMUNITY.<br/>
            <span className="text-ep-gray">NOT EVERY COMMUNITY MEMBER IS PART OF THE FORUM.</span>
          </h2>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-ep-border border border-ep-border">
      ...
      <RevealSection className="px-6 md:px-12 lg:px-24 py-24 border-b border-ep-border bg-ep-bg">
        <div className="max-w-6xl mx-auto">
          <div className="mb-16">
            <span className="font-mono text-xs text-ep-gray tracking-[0.2em] uppercase mb-4 block">02 // OPERATIONS</span>
            <h2 className="text-4xl md:text-5xl font-mono uppercase tracking-tight text-ep-black">What the Forum Does</h2>
          </div>
          
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px bg-ep-border border border-ep-border">
      ...
      <RevealSection className="px-6 md:px-12 lg:px-24 py-24 border-b border-ep-border bg-ep-bg">
        <div className="max-w-5xl mx-auto grid md:grid-cols-2 gap-16 md:gap-8">
          <div>
            <span className="font-mono text-xs text-ep-gray tracking-[0.2em] uppercase mb-4 block">03 // QUALIFICATIONS</span>
            <h2 className="text-4xl md:text-5xl font-mono uppercase tracking-tight text-ep-black mb-8 leading-tight">
              You do not need to be the strongest chess player in the room.
            </h2>
            <div className="space-y-4">
              {applicantQualities.map((q, i) => (
                <div key={i} className="font-mono text-xs uppercase tracking-wider text-ep-gray flex items-start gap-4 opacity-0 motion-safe:animate-reveal-up motion-reduce:opacity-100" style={{ animationDelay: `${150 + (100 * i)}ms` }}>
                  <span className="text-ep-black mt-1" aria-hidden="true">✓</span>
                  <p>{q}</p>
                </div>
              ))}
            </div>
          ...
          <div className="chess-cell-dark interactive-active p-8 md:p-12 border border-ep-border flex flex-col justify-center opacity-0 motion-safe:animate-reveal-up motion-safe:[animation-delay:650ms] motion-reduce:opacity-100">
      ...
      <RevealSection className="px-6 md:px-12 lg:px-24 py-24 border-b border-ep-border bg-ep-bg">
        <div className="max-w-5xl mx-auto">
          <div className="mb-16">
            <span className="font-mono text-xs text-ep-gray tracking-[0.2em] uppercase mb-4 block">04 // TIMELINE</span>
            <h2 className="text-4xl md:text-5xl font-mono uppercase tracking-tight text-ep-black">Recruitment Process</h2>
          </div>
          
          <div className="flex flex-col gap-px bg-ep-border border border-ep-border">
            {recruitmentProcess.map((step, i) => (
              <div key={i} className="chess-cell-dark p-6 md:p-8 flex flex-col md:flex-row md:items-center gap-6 group relative opacity-0 motion-safe:animate-reveal-up motion-reduce:opacity-100" style={{ animationDelay: `${150 * (i + 1)}ms` }}>
                <span className="font-mono text-3xl font-bold text-ep-gray opacity-40 w-16 group-hover:opacity-100 transition-opacity">
                  {step.step}
                </span>
      ...
      <RevealSection className="px-6 md:px-12 lg:px-24 py-24 border-b border-ep-border bg-ep-bg-deep text-center md:text-left">
        <div className="max-w-3xl mx-auto chess-cell-dark border border-ep-border p-8 md:p-12 relative group opacity-0 motion-safe:animate-reveal-up motion-safe:[animation-delay:150ms] motion-reduce:opacity-100">
      ...
      {/* 06 - FINAL CTA */}
      <RevealSection className="px-6 md:px-12 lg:px-24 py-32 bg-[#080808] text-center relative" animationClass="motion-safe:animate-reveal-up">
        <div className="max-w-4xl mx-auto flex flex-col items-center">
          <h2 className="text-4xl md:text-5xl lg:text-7xl font-mono uppercase tracking-tight text-[#FAF8F2] mb-12 leading-none">
            THE BOARD DOESN&apos;T<br/>MOVE ITSELF.
          </h2>
          
          <div className="font-mono text-sm uppercase tracking-widest text-[#FAF8F2]/70 space-y-3 mb-16">
            ...
            <p className="text-[#FAF8F2] mt-8 pt-8 border-t border-white/20 opacity-0 motion-safe:animate-reveal-up motion-safe:[animation-delay:900ms] motion-reduce:opacity-100">THE NEXT SOMEONE COULD BE YOU.</p>
          </div>

          <div className="bg-[#111111] border border-white/10 p-8 md:p-12 w-full max-w-xl opacity-0 motion-safe:animate-fade-in motion-safe:[animation-delay:1050ms] motion-reduce:opacity-100">
             <div className="bg-[#080808] border border-white/20 px-8 py-4 w-full cursor-not-allowed">
               <span className="font-mono text-sm tracking-widest uppercase text-[#FAF8F2] block text-center opacity-70">
                 APPLICATION FORM COMING NEXT PHASE
               </span>
             </div>
          </div>
