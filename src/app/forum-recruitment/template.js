...
export default function RecruitmentTemplate({ children }) {
  ...
  return (
    <div className={`theme-light bg-surface-primary min-h-screen ${!hasJustTransitioned ? 'motion-safe:animate-clip-down motion-safe:origin-top motion-reduce:clip-path-none' : ''}`}>
      {children}
    </div>
  );
}
